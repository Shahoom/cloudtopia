...
export default function EcommerceSolutionsClient({ t: pageT }: { t?: any }) {
  ...
  // Safely map sections and gallery using CMS dictionary overrides if they exist
  const sections = p?.sections || currentContent.sections
  const gallery = p?.gallery || currentContent.gallery
  ...
  return (
    <main className="flex-grow">
      ...
      >
            ...
            <ContainerScroll
              titleComponent={
                <div className="space-y-4 sm:space-y-6 md:space-y-8 mb-8 sm:mb-12 md:mb-16">
                  ...
                  <span className={`inline-block px-4 py-2 rounded-full text-sm sm:text-base font-semibold text-white bg-gradient-to-r ${accentColors[index % accentColors.length]}`}>
                    {section.subtitle}
                  </span>
                  ...
                  <p className={`text-3xl sm:text-4xl md:text-5xl lg:text-6xl xl:text-7xl font-black bg-gradient-to-r ${accentColors[index % accentColors.length]} bg-clip-text text-transparent leading-tight`}>
                    {section.highlight}
                  </p>
                </div>
              }
        ...
        <DetailedServicesSection pillarSlug="ecommerce-development" locale={locale === 'ar' ? 'ar' : 'en'} />
      </div>
    </main>
  )
}
