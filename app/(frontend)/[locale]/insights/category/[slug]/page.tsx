...
export async function generateMetadata({ params }: PageProps): Promise<Metadata> {
  ...
  const title = category.seo?.metaTitle || `${category.name} Insights`
  ...
  return {
    ...
    alternates: {
      canonical: category.seo?.canonicalUrl || canonicalUrl(locale, `/insights/category/${category.slug}`),
      languages: buildHreflangMap(`/insights/category/${category.slug}`),
    },
  }
}

export default async function InsightCategoryPage({ params, searchParams }: PageProps) {
  const { locale = 'en', slug } = await params
    ...
    <div className="min-h-screen bg-[#f4f1f8] px-4 pb-20 pt-28 sm:px-6 lg:px-8">
      <div className="mx-auto max-w-7xl">
        ...
        <header className="mb-10 rounded-3xl border border-white/80 bg-white p-8 shadow-xl shadow-sky-950/10 md:p-10">
          <p className="text-sm font-black uppercase tracking-normal text-primary-700">Category</p>
          ...
          <div className="mt-8">
            <BlogSearch
              action={localePath(locale, `/insights/category/${category.slug}`)}
              defaultValue={search}
              label={`Search ${category.name} insights`}
            />
          </div>
        </header>
        <BlogGrid
          ...
          locale={locale}
          emptyTitle={`No ${category.name} insights yet`}
          emptyText="Published articles for this category will appear here."
        />
        <BlogPagination
          basePath={localePath(locale, `/insights/category/${category.slug}`)}
