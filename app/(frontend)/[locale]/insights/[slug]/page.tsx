...
export async function generateMetadata({ params }: PageProps): Promise<Metadata> {
  ...
  const canonical = post.seo.canonicalUrl || canonicalUrl(locale, `/insights/${post.slug}`)
  ...
  return {
    ...
    alternates: {
      canonical,
      languages: buildHreflangMap(`/insights/${post.slug}`),
    },
  }
}
...
export default async function InsightArticlePage({ params }: PageProps) {
  ...
  const articleSchema = {
    ...
    author: {
      ...
      url: post.author?.slug ? canonicalUrl(locale, `/insights/author/${post.author.slug}`) : canonicalUrl(locale, '/'),
    },
    publisher: {
      '@type': 'Organization',
      name: 'CloudTopia',
  ...
  const breadcrumbSchema = {
    ...
    itemListElement: [
      ...
      { '@type': 'ListItem', position: 2, name: 'Insights', item: canonicalUrl(locale, '/insights') },
