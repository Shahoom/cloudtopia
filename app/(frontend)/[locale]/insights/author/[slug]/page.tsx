...
export async function generateMetadata({ params }: PageProps): Promise<Metadata> {
  ...
  const title = author.seo?.metaTitle || `${author.name} | CloudTopia Insights`
  ...
  return {
    ...
    alternates: {
      canonical: canonicalUrl(locale, `/insights/author/${author.slug}`),
      languages: buildHreflangMap(`/insights/author/${author.slug}`),
    },
  }
}

export default async function InsightAuthorPage({ params }: PageProps) {
  const { locale = 'en', slug } = await params
  const author = await getBlogAuthor(locale, slug)
  if (!author) notFound()
      ...
      <div className="mx-auto max-w-7xl">
        ...
        <header className="mb-10 rounded-[2rem] border border-white/80 bg-white p-8 shadow-xl shadow-sky-950/10 md:p-10">
          <div className="grid gap-8 md:grid-cols-[140px_1fr] md:items-center">
            ...
            <div>
              <p className="text-sm font-black uppercase tracking-normal text-primary-700">Author</p>
        ...
        <div className="mb-8">
          <p className="text-sm font-black uppercase tracking-normal text-primary-700">Articles</p>
          <h2 className="mt-2 text-3xl font-black tracking-normal text-neutral-950 md:text-5xl">Insights by {author.name}</h2>
        </div>
        <BlogGrid posts={posts} locale={locale} emptyTitle="No articles yet" emptyText="Published author articles will appear here." />
        <div className="mt-16">
          <NewsletterBox />
        </div>
      </div>
