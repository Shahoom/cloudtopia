...
export default async function LocaleLayout({
    ...
    // The retired fabricated testimonial (inside the demo-request modal copy)
    // was removed from the code dictionaries, but the CMS chrome bundle still
    // merges a stored copy back in. Strip it here — the single point every
    // page's dictionary passes through — so no CMS state can reintroduce it
    // into page payloads.
    {
        const heroModal = (chromeDictionary as any).home?.hero?.modal
        if (heroModal) {
            delete heroModal.testimonial
            delete heroModal.testimonialAuthor
            delete heroModal.testimonialRole
        }
    }
