...
export default function ProjectsPageClient({ t: pageT }: { t?: any }) {
  ...
  const projectUi = locale === 'ar'
    ? {
      searchLabel: 'البحث في المشاريع',
      emptyTitle: 'لم يتم العثور على مشاريع',
      emptyDescription: 'جرّب كلمة بحث أخرى أو غيّر فلتر التصنيف.',
    }
    : {
      searchLabel: 'Search projects',
      emptyTitle: 'No projects found',
      emptyDescription: 'Try another search term or switch the category filter.',
    }
        ...
        <div className="container max-w-7xl mx-auto px-4 relative">
          ...
          >
            <div className="mx-auto mb-8 max-w-xl">
              <label htmlFor="project-search" className="sr-only">{projectUi.searchLabel}</label>
          ...
          {filteredProjects.length === 0 && (
            <div className="mx-auto mt-12 max-w-xl rounded-3xl border border-neutral-200 bg-white p-8 text-center shadow-lg">
              <h3 className="text-2xl font-bold text-neutral-900">{projectUi.emptyTitle}</h3>
              <p className="mt-3 text-neutral-600">{projectUi.emptyDescription}</p>
            </div>
          )}
        </div>
      </section>
