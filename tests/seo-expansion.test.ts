...
test('phase 2 and 3 landing page templates surface the expanded SEO content', () => {
  ...
  const industrySharedSource = readFileSync(path.join(process.cwd(), 'app/(frontend)/[locale]/industries/_shared/industry-route.tsx'), 'utf8')
  ...
  assert.match(industrySharedSource, /getIndustryPage/, 'Industry route should use the closed world resolver')
  assert.match(industryPageSource, /IndustryPageShell/, 'Industry route should render registered world pages')
  assert.match(industrySharedSource, /buildIndustryMetadata/, 'Industry route should use centralized metadata')
  assert.match(industrySharedSource, /buildIndustryJsonLd/, 'Industry route should use connected industry schema')
