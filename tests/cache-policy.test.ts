...
import { CMS_REVALIDATE_SECONDS, FEED_REVALIDATE_SECONDS } from '../lib/cms/cache-policy.ts'
 ...
 * configuration export detected". So each route hardcodes the number and this
 * test is what keeps them honest.
 */
const readRevalidate = (relativePath: string): number => {
...
test('page routes revalidate on the shared CMS policy', () => {
  const pageRoutes = [
  ...
  for (const route of pageRoutes) {
    assert.equal(
      ...
      `${route} drifted from CMS_REVALIDATE_SECONDS in lib/cms/cache-policy.ts`,
    )
  }
})
