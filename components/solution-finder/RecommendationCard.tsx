...
export default function RecommendationCard({ recommendation, answers, isComplete, locale, t, onCtaClick }: Props) {
  ...
  return (
    <div className="h-full flex flex-col rounded-3xl bg-[#0b0f19] border border-white/10 overflow-hidden relative shadow-xl shadow-[rgba(15,23,42,0.06)] min-h-[390px]">
      {/* Ambient glows */}
      <div className="absolute inset-0 pointer-events-none">
        <div className="absolute -top-24 -end-24 w-72 h-72 rounded-full bg-[#6366f1]/10 blur-3xl" />
        <div className="absolute -bottom-24 -start-24 w-72 h-72 rounded-full bg-indigo-500/8 blur-3xl" />
      </div>

      {/* Floating Microchip Icon at top-right */}
      <div className="absolute top-6 end-6 w-10 h-10 rounded-xl bg-[#6366f1]/10 border border-[#6366f1]/25 flex items-center justify-center shadow-lg shadow-[#6366f1]/10">
          ...
          {isPopulated ? (
            ...
            >
              <div className="flex items-center gap-2 mt-3">
                ...
                <span className="text-white/60 text-xs">{recommendation.matchScore}% {t.card.aligned}</span>
              </div>
              <h3 className="text-lg font-bold text-white mt-2 leading-snug">
                {recommendation.packageTitle}
              </h3>
              <p className="text-xs text-white/50 mt-1 leading-snug">{recommendation.packageSubtitle}</p>
            </motion.div>
          ) : (
            <motion.div key="placeholder-header" initial={{ opacity: 0 }} animate={{ opacity: 1 }} exit={{ opacity: 0 }}>
              <h3 className="text-[17px] font-bold text-white mt-3 leading-snug max-w-[210px]">
      ...
      <div className="relative flex-1 px-5 py-4 space-y-4 overflow-y-auto">
        <AnimatePresence mode="wait">
          {isPopulated ? (
            ...
            >
              {/* Intro */}
              <p className="text-[13px] sm:text-[14px] text-white/90 leading-relaxed font-semibold">
                {recommendation.personalizedIntro}
              </p>

              {/* Meta rows */}
              <div className="space-y-2.5 pt-1.5">
                <MetaRow icon={<Calendar className="w-4 h-4 text-[#6366f1]" />} label={t.card.timeline} value={recommendation.estimatedTimeline} />
                <MetaRow icon={<DollarSign className="w-4 h-4 text-emerald-400" />} label={t.card.budget} value={recommendation.budgetRange} />
                <MetaRow icon={<Code2 className="w-4 h-4 text-violet-400" />} label={t.card.tech} value={recommendation.techStack.slice(0, 3).join(', ')} />
              </div>

              {/* Key features (top 4) */}
              <div className="pt-1.5">
                <p className="text-xs font-bold text-[#6366f1] uppercase tracking-widest mb-2">{t.card.features}</p>
                <ul className="space-y-1.5">
                  {recommendation.keyFeatures.slice(0, 4).map((f) => (
                    <li key={f} className="flex items-start gap-2 text-[13px] sm:text-[14px] text-white/90 font-medium leading-tight">
              ...
              {/* Services tags */}
              <div className="pt-1.5">
                <p className="text-xs font-bold text-[#6366f1] uppercase tracking-widest mb-1.5">{t.card.services}</p>
                <div className="flex flex-wrap gap-1.5">
                  {recommendation.recommendedServices.slice(0, 4).map((s) => (
                    <span key={s} className="px-2.5 py-1 rounded-lg bg-white/10 border border-white/15 text-white/90 text-xs font-semibold leading-none">
                      {s}
                    </span>
                  ))}
                </div>
            ...
            <motion.div key="placeholder" initial={{ opacity: 0 }} animate={{ opacity: 1 }} exit={{ opacity: 0 }} className="space-y-3.5">
              {[
                { icon: <Calendar className="w-4 h-4 text-[#6366f1]/60" />, label: t.card.timeline, value: '—' },
                { icon: <DollarSign className="w-4 h-4 text-emerald-400/60" />, label: t.card.budget, value: '—' },
                { icon: <Code2 className="w-4 h-4 text-violet-400/60" />, label: t.card.tech, value: locale === 'ar' ? 'بناءً على قطاع عملك' : 'Based on your industry' },
