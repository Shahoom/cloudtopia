...
const ProceduralGroundBackground: React.FC = () => {
  ...
  useEffect(() => {
    ...
    const fsSource = `
      ...
      void main() {
        ...
        // Color Palette
        vec3 baseColor = vec3(0.05, 0.04, 0.12); // Slightly brighter deep sky
        vec3 accentColor = vec3(0.1, 0.2, 0.6);   // More vibrant blue
        vec3 neonColor = vec3(0.5, 0.3, 0.9);     // Brighter neon purple
        ...
        // Horizon Fog / Fade
        float fade = smoothstep(0.6, -0.6, uv.y);
        finalColor *= (1.0 - length(uv) * 0.3) * (1.0 - fade);
  ...
  return (
    <div className="absolute inset-0 w-full h-full bg-[#030208] pointer-events-none overflow-hidden">
      <canvas
        ref={canvasRef}
