...
function splitText(text: string) {
  // For Arabic text, keep words intact to preserve letter connections
  if (isArabic(text)) {
    const words = text.split(" ").filter(word => word.length > 0)
    return {
      words,
      characters: words, // Use words instead of characters for Arabic
      isArabicText: true,
    }
  }
...
>(({ text, index, children, className, ...props }, ref) => {
  ...
  const { characters, isArabicText } = splitText(text)
    ...
    >
      {characters.map((char, idx) => (
        ...
        >
          <MotionConfig
            transition={{
              delay: isArabicText ? idx * 0.05 : idx * 0.025,
              duration: 0.3,
          ...
          >
            ...
            >
              {char}
              {isArabicText && idx < characters.length - 1 && <>&nbsp;</>}
              {!isArabicText && char === " " && idx < characters.length - 1 && <>&nbsp;</>}
            </motion.span>
