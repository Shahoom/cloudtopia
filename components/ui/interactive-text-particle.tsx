...
const ParticleTextEffect: React.FC<ParticleTextEffectProps> = ({
    ...
    const write = () => {
        ...
        // Start with a size based on canvas height
        let fontSize = Math.floor(canvas.height * 0.5);
        ...
        let measuredWidth = ctx.measureText(textBox.str).width;

        // Target: text should fill 85% of canvas width
        const targetWidth = canvas.width * 0.85;

        // Scale font size to fit target width
        if (measuredWidth > 0) {
            fontSize = Math.floor(fontSize * (targetWidth / measuredWidth));
        }

        // Cap font size to max 60% of canvas height so it doesn't overflow vertically
        const maxFontSize = Math.floor(canvas.height * 0.6);
        ...
        ctx.font = `900 ${textBox.h}px ${fontFamily}`;
        textBox.w = Math.round(ctx.measureText(textBox.str).width);

        interactionRadiusRef.current = Math.max(20, textBox.h * 0.8);
