...
export function DynamicTextSlider({ 
  ...
  subtitle = "An intuitive, powerful approach to content creation. We craft stunning visuals, videos, and copy that captivate your audience.",
  locale = "en"
}: TitleComponentProps) {
    ...
    <div className="w-full min-h-screen bg-lavender dark:bg-eerie text-white flex flex-col items-center justify-center text-center p-4 font-sans" dir={isRTL ? 'rtl' : 'ltr'}>
      <div className="max-w-5xl">
        ...
        <div className="flex justify-center gap-4 mt-4 md:mt-6">
          <ContentSlider width={textWidth} text={sliderText} isRTL={isRTL} />
        </div>

        {/* Subheading */}
        <p className="mt-8 text-lg md:text-xl max-w-2xl mx-auto text-gray-500 dark:text-gray-400">
          {subtitle}
        </p>
      </div>
    </div>
 ...
 */
function ContentSlider({ width: initialWidth, height: propHeight, handleSize = 28, onChange, text = "Content Creation", isRTL = false }: SliderProps) {
  // Slightly more height for Arabic text only
  const height = propHeight ?? (isRTL ? 135 : 70);
  ...
  return (
    ...
    >
      <div className="absolute inset-0 rounded-2xl border border-purple-500 pointer-events-none" />
