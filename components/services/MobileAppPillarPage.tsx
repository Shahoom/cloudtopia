...
export default async function MobileAppPillarPage({ locale }: { locale: string }) {
    ...
    const subServices: { name: string; desc: string; icon: string; href: string }[] = [
        ...
        { name: t('Cross-Platform Apps', 'تطبيقات متعددة المنصات'), desc: t('One React Native or Flutter codebase shipping to both stores — near-native, half the cost.', 'كود واحد بـ React Native أو Flutter يصل للمتجرين — أداء شبه أصلي بنصف التكلفة.'), icon: 'Layers', href: '/services/app-development/cross-platform-app-development' },
