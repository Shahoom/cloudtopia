...
type EducationPricingTabsProps = {
  projectLabel: string
  retainerLabel: string
  projectPlans: readonly EducationPlan[]
  retainerPlans: readonly EducationPlan[]
  tabsLabel: string
  ctaLabel: string
  whatsappHref: string
  direction: 'ltr' | 'rtl'
...
/**
 * Engagement-model tabs — a React port of Learnit's pricing Monthly/Yearly tab
 * switch, repurposed to CloudTopia's Project / Retainer engagement models. Both
 * panels stay in the DOM; the inactive one is `hidden` (display:none), so
 * un-hiding a panel restarts its fade. Full ARIA tab pattern with roving
 * tabindex and RTL-aware arrow keys.
 */
export function EducationPricingTabs({
  projectLabel,
  retainerLabel,
  projectPlans,
  retainerPlans,
  ...
  const renderPanel = (key: TabKey, plans: readonly EducationPlan[]) => (
    <div
