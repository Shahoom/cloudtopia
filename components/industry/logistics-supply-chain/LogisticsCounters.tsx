...
/**
 * counterUp-style stat roll-ups. An IntersectionObserver (threshold 0.3) fires
 * the count-up exactly once, mirroring the Logistick counterUp signature.
 * Numbers render as plain integers server-side (no hydration mismatch), then
 * the client formats them per locale (Arabic-Indic digits for `ar`) and
 * animates 0 -> value. Under prefers-reduced-motion the final value shows
 * immediately.
 */
export function LogisticsCounters({ stats, locale, caption, note }: LogisticsCountersProps) {
