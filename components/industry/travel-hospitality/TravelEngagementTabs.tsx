...
import type { TravelPlan } from './travel-hospitality-content'
import styles from './travel-hospitality-industry.module.css'

type TravelEngagementTabsProps = {
  projectLabel: string
  retainerLabel: string
  projectPlans: readonly TravelPlan[]
  retainerPlans: readonly TravelPlan[]
  tabsLabel: string
  ctaLabel: string
  whatsappHref: string
  direction: 'ltr' | 'rtl'
...
/**
 * Engagement-model tabs (repurposed from the template's best-packages / pricing
 * blocks). Both panels stay in the DOM; the inactive one is `hidden`
 * (display:none), so un-hiding a panel restarts its fade animation. Full ARIA tab
 * pattern with roving tabindex and RTL-aware arrow keys.
 */
export function TravelEngagementTabs({
  projectLabel,
  retainerLabel,
  projectPlans,
  retainerPlans,
  ...
  const renderPanel = (key: TabKey, plans: readonly TravelPlan[]) => (
    <div
