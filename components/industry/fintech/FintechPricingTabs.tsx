...
type FintechPricingTabsProps = {
  projectLabel: string
  retainerLabel: string
  projectPlans: readonly FintechPlan[]
  retainerPlans: readonly FintechPlan[]
  tabsLabel: string
  ctaLabel: string
  whatsappHref: string
  direction: 'ltr' | 'rtl'
...
/**
 * Monthly/Annual-style pricing tabs, repurposed to CloudTopia engagement models
 * (Project / Retainer). Reproduces Paynext's jQuery fade: both panels stay in
 * the DOM; the inactive one is `hidden` (display:none), so un-hiding a panel
 * restarts its `fintechFade` animation. Full ARIA tab pattern with roving
 * tabindex and RTL-aware arrow keys.
 */
export function FintechPricingTabs({
  projectLabel,
  retainerLabel,
  projectPlans,
  retainerPlans,
  ...
  const renderPanel = (key: TabKey, plans: readonly FintechPlan[]) => (
    <div
