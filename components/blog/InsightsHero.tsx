...
export function InsightsHero({
  ...
  return (
    <section className="relative overflow-hidden px-4 pb-16 pt-28 sm:px-6 lg:px-8" data-header-theme="light">
      <div className="absolute inset-0 bg-[#f4f1f8]" />
      <div className="absolute inset-0 bg-[linear-gradient(rgba(2,132,199,0.09)_1px,transparent_1px),linear-gradient(90deg,rgba(2,132,199,0.09)_1px,transparent_1px)] bg-[size:44px_44px]" />
      ...
      <div className="relative mx-auto grid max-w-7xl min-w-0 gap-10 lg:grid-cols-[minmax(0,1.04fr)_minmax(420px,0.96fr)] lg:items-center">
        <div className="min-w-0">
          <span className="mb-6 inline-flex items-center gap-2 rounded-full border border-sky-100 bg-white px-4 py-2 text-sm font-black text-primary-700 shadow-sm">
            <Sparkles className="h-4 w-4" />
            CloudTopia editorial hub
          </span>
          ...
          <form action={localePath(locale, '/insights/search')} className="mt-9 max-w-2xl">
            <label className="sr-only" htmlFor="insights-hero-search">
              Search CloudTopia Insights
            </label>
            <div className="flex min-h-16 min-w-0 flex-col gap-3 rounded-2xl border border-white/80 bg-white p-2 shadow-2xl shadow-sky-950/12 sm:flex-row sm:items-center">
              <Search className="ml-3 mt-2 h-5 w-5 flex-none text-primary-700 sm:mt-0" />
              <input
                id="insights-hero-search"
                name="q"
                defaultValue={search}
                placeholder="Search websites, CRM, dashboards, AI automation..."
          ...
          <div className="mt-8 grid max-w-3xl min-w-0 grid-cols-1 gap-3 min-[360px]:grid-cols-2 sm:grid-cols-4">
            {focusCards.map(({ label, icon: Icon }) => (
              <div key={label} className="min-w-0 rounded-2xl border border-white/80 bg-white/80 p-4 shadow-sm backdrop-blur">
                <Icon className="mb-3 h-5 w-5 text-primary-700" />
                <p className="text-sm font-black leading-5 text-neutral-900">{label}</p>
              </div>
            ))}
          </div>
        </div>
        ...
        <div className="relative min-h-[480px] min-w-0 overflow-hidden">
          ...
          <div className="absolute left-8 right-8 top-8 rounded-3xl border border-sky-100 bg-white p-5 shadow-xl">
            <div className="flex items-center justify-between gap-4">
              <div>
                <p className="text-xs font-black uppercase tracking-normal text-primary-700">Editorial system</p>
                <h2 className="mt-2 text-2xl font-black tracking-normal text-neutral-950">Smarter publishing for digital growth</h2>
              </div>
              <span className="rounded-full bg-emerald-50 px-3 py-1 text-xs font-black text-emerald-700">Live</span>
            </div>
            <div className="mt-6 grid gap-3">
              {['AI-assisted outlines', 'SEO-ready article schema', 'Custom content blocks', 'Lead-focused CTAs'].map((item, index) => (
                <div key={item} className="flex items-center gap-3 rounded-2xl bg-[#f4f1f8] p-3">
          ...
          <div className="absolute bottom-10 left-5 right-16 rounded-3xl border border-white/90 bg-neutral-950 p-5 text-white shadow-2xl">
            <p className="text-xs font-black uppercase tracking-normal text-sky-200">Reader journey</p>
            <div className="mt-4 grid grid-cols-3 gap-3">
              {['Discover', 'Learn', 'Start project'].map((item) => (
